          ...
          <div class="relative flex-row flex-1 w-full max-w-full px-4">
            ...
            <div class="relative grid grid-cols-6 gap-6 mt-2 ">
              ...
              <div class="col-span-3 md:col-span-2 lg:col-span-1">
                <x-jet-label class="text-xs" for="select" value="{{ __('app.By') }} {{ __('role.role') }}" />
              ...
              <div class="col-span-3 md:col-span-2 lg:col-span-1">
                <x-jet-label class="text-xs" for="select" value="{{ __('app.By') }} {{ __('country.country') }}" />
                  ...
                  <tr
                      ...
                      <div class="flex items-center text-sm">
                        ...
                        <div class="relative hidden w-10 h-10 rounded-full ltr:mr-3 rtl:ml-3 md:block">
                          <img class="object-cover w-full h-full rounded-full" src="{{ $user->profile_photo_url }}"
                            alt="..." />
                          <div
                            class="absolute bottom-0 w-2 h-2 {{ $user->isOnline() ? 'bg-green-500' : 'bg-gray-500' }} rounded-full ltr:left-0 rtl:right-0 ltr:ml-1 rtl:mr-1">
                          </div>
                    ...
                    <td class="px-4 py-3">
                      <div class="flex items-center justify-between gap-1 text-sm text-center">
                        ...
                        @if ($trashed)
                          @can('restore', $user)
                            <x-jet-button wire:click="selectedItem('restore',{{ $user->id }})" class="px-2">
                              <x-svg.svg-restore class="w-5 h-5" />
                            </x-jet-button>
                          @endcan

                          @can('forceDelete', $user)
                            <x-jet-button wire:click="selectedItem('forceDelete',{{ $user->id }})" class="px-2">
                              <x-svg.svg-force-delete class="w-5 h-5" />
                            </x-jet-button>
                          @endcan
                        @else
                          @can('update', $user)
                            <x-jet-button wire:click="selectedItem('update',{{ $user->id }})" class="px-2">
                          ...
                          @can('view', $user)
                            <x-jet-button wire:click="selectedItem('show',{{ $user->id }})" class="px-2">
                          ...
                          @can('delete', $user)
                            <x-jet-button wire:click="selectedItem('delete',{{ $user->id }})" class="px-2">
